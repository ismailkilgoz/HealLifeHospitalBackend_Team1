Feature: An administrator (admin) should be able to create a new visitor record via API connection.
  ...
  Scenario: Invalid Token and No Data, When a POST body, with invalid authorization information and  without any data, is sent to the /api/visitorsAdd endpoint,
  ...
  the message in the response body is "No information or missing information. Please check your input data.".
    ...
    * An api user sends a "POST" request with a body with invalid token and saves the returned response.
  ...
  (purpose, name, email, contact, id_proof, visit_to, ipd_opd_staff_id, related_to, no_of_pepple, date, in_time, out_time, note)
  to /api/visitorsAdd endpoint, it should be verified that the status code returned is 203 and the message information in the response body is “You do not have authorization or token error.”.
    ...
    * An api user verifies that the returned status code is 203.
    # Api kullanicisi status codeun 203 oldugunu dogrular
    * An api user verifies that the "message" information in the response body is "You do not have authorization or token error.".
    # Api kullanicisi response body'deki message bilgisinin "You do not have authorization or token error." oldugunu dogrular
    Examples:
      | purpose   | name       | email           | contact  | id_proof | visit_to    | ipd_opd_staff_id | related_to      | no_of_pepple | date       | in_time  | out_time | note                        |
      | deneme 21 | Karan Ates | karan@gmail.com | 14068377 | 20240118 | ipd_patient | 7                | umit ureten(83) | 1            | 2023-10-05 | 09:30 PM | 10:30 PM | please check your mail box! |
