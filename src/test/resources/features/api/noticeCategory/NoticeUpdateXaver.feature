Feature: As an administrator (admin), I should be able to update the announcement information registered in the system via API connection.
  ...
  request to the "/api/getNoticeById" endpoint using the "updateId" from the response.
    ...
    * The api user verifies that the updatedid information in the Response body is the same as the id information in the patch request body
  ...
  number." should be verified.
    ...
    * The api user verifies that the "message" information in the response body is "Wrong information or missing information. Please check your input data and id number.".
    ...
    Examples:
      | id  | type   | title           | description   | slug           |
      | 4099 | notice | notice hospital | notice deneme | noticehospital |



  Scenario Outline: When a PATCH body (id, type, title, description and slug) containing valid authorization information
  and invalid (id) is sent to the /api/updateNotice endpoint, the status code returned is 203 and the message
  information in the response body is "Wrong information or missing information. Please check your input data and id
  number." should be verified.
    ...
    * The api user sets "api/updateNotice" path parameters.
    # Api kullanicisi "api/visitorsPurposeUpdate" path parametrelerini olusturur
    * The api user prepares a PATCH request containing "<type>", "<title>", "<description>" and "<slug>" information to send to the api updateNotice endpoint.
    # Api kullanicisi api visitorsPurposeUpdate endpointine gondermek icin "<visitors_purpose>" ve "<description>" bilgilerini iceren bir patch request hazirlar
    * The api user sends a PATCH request and saves the returned response.
    # Api kullanicisi PATCH request gonderir ve donen responsei kaydeder
    * The api user verifies that the status code is 203.
  ...
  * The api user prepares a Get request containing the <id> information to send to the api getNoticeById endpoint.
    ...
    * The api user verifies that the "lists.title" information in the response body is "notice hospital".
